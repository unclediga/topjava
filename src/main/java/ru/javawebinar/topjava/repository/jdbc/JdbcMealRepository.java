...
public class JdbcMealRepository implements MealRepository {
    ...
    public List<Meal> getAll(int userId) {
        return jdbcTemplate.query("SELECT id,datetime,calories,description " +
                "FROM meals WHERE user_id=? ORDER BY datetime DESC", ROW_MAPPER, userId);
    }

    @Override
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT id,datetime,calories,description " +
                "FROM meals WHERE user_id=? AND datetime BETWEEN ? AND ?" +
